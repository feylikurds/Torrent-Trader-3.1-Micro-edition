...
$debug = 0;
	...
	if ($type == 'torrent')
	{
		...
		if ($count_tid > 0)
		{
			...
			print("<div style='margin-top:10px; margin-bottom:5px'><a href=torrents-details.php?id=$tid><b><input type='submit' value='".T_("BACK_TO_TORRENT")."'></b></a></div>");
			...
			print("<div style='margin-top:5px; margin-bottom:10px' align='right'><a href=torrents-details.php?id=$tid><b><input type='submit' value='".T_("BACK_TO_TORRENT")."'></b></a></div>");
				endif;
				end_frame();
				stdfoot();
				die;
	...
	<br /><table class="f-border" align='center' cellpadding='0' cellspacing='3' width='100%'><tr><td>
		...
		<!-- &nbsp;|&nbsp;
		<?php print("<a href='mybonus.php'><b>".T_("SEEDBONUS")."</b></a>");?> -->
		</td></tr>
		</table>
	</td></tr></table>
	<?php
			...
			if ( $uid != $CURUSER['id'] )
				print("<div style='margin-top:10px; margin-bottom:5px'><a href=account-details.php?id=$uid><b><input type='submit' value='".T_("GO_TO_USER_ACCOUNT")."'></b></a></div>");
			...
			<table border="0" class="table_table" cellpadding="4" cellspacing="0" width="100%">
					...
					?>
					<tr align="center">
						<?php echo("<td class='ttable_col1' align='left' nowrap='nowrap'>".(is_countable($expandrows)?"<a href=\"javascript: klappe_torrent('t".$row['0']."')\"><img border=\"0\" src=\"".$site_config["SITEURL"]."/images/plus.gif\" id=\"pict".$row['0']."\" alt=\"Show/Hide\" class=\"showthecross\" /></a>":"")."<a title=\"".$row["1"]."\" href=\"torrents-details.php?id=".$row['0']."&amp;hit=1\"><b>$smallname</b></a> $freeleech</td>"); ?>
					  <?php if ($site_config["ALLOWEXTERNAL"]) { ?>
			...
			if ( $uid != $CURUSER['id'] )
				print("<div style='margin-top:5px; margin-bottom:10px' align='right'><a href=account-details.php?id=$uid><b><input type='submit' value='".T_("GO_TO_USER_ACCOUNT")."'></b></a></div>");
		...
		else
		{
			...
			{
				if ( $uid != $CURUSER[id] ) {
					print("<div style='margin-bottom:10px' align='center'><a href=account-details.php?id=$uid><b><input type='submit' value='".T_("GO_TO_USER_ACCOUNT")."'></b></a></div>");
				} else {
