...
echo ("<b>Total HDD: </b>" . round(disk_free_space($fs) / (1024*1024*1024)) . " GB\r\n<br />");
// calculate used space
$disk_used_space = round((disk_total_space($fs) - disk_free_space($fs)) / (1024*1024*1024)); 
echo ("<b>HDD Used: </b>" . $disk_used_space . " GB\r\n<br />");
// calculate % used space
echo ("<b>Percentage: </b>" . round((disk_total_space($fs) - disk_free_space($fs)) / disk_total_space($fs) * 100) . " %<br/><hr/>");


	//echo("<b>Current Users:</b> $users[0]<br>
	echo("<b>CPU Info: </b> $processor1<br /><hr />");	
	echo("<b>Local Server Time:</b> $localtime<br /><hr />");
	
    $dt = new DateTime();
	echo ("<b>PHP UTC Time: </b>".gmdate('m-d-Y H:i:s')."<br /><hr />");
