...
//END//
	...
	$maxinvites[1] = 2;   // User
	$maxinvites[2] = 5;  // Power User
	$maxinvites[3] = 5;  // VIP
	$maxinvites[4] = 5;  // Uploader
	$maxinvites[5] = 10; // Moderator
	$maxinvites[6] = 10; // Super Moderator
	$maxinvites[7] = 20; // Administrator
	$maxinvites[8] = 40; // Site Owner
  ...
  $deadtime = gmtime() - $site_config["invite_timeout"]; 
  $res = SQL_Query_exec("SELECT u.id, u.invited_by, u.added, i.invitees FROM users u LEFT JOIN users i ON u.invited_by = i.id WHERE u.status = 'pending' AND u.invited_by != '0' AND u.added < FROM_UNIXTIME($deadtime)");
  while ($row = mysqli_fetch_assoc($res))
  {     
