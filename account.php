...
//
//  TorrentTrader v2.x
//      $LastChangedDate: 2011-11-27 18:16:07 +0000 (Sun, 27 Nov 2011) $
...
?>
	<br /><table class="f-border" align='center' cellpadding='0' cellspacing='3' width='100%'><tr><td>
		...
		<!-- &nbsp;|&nbsp;
		<?php print("<a href='mybonus.php'><b>".T_("SEEDBONUSPROFILE")."</b></a>");?> -->
		</td></tr>
		</table>
	</td></tr></table>
	<?php
...
//////////END H&R SYSTEM EDITS///////////////
	...
	<?php echo("<b>".T_("IP").":</b> " . $CURUSER["ip"] . "\n"); ?><br />

    <a href="uptransfer.php"><b><?php echo T_("DONATE_UPLOAD_CREDITS"); ?></b></a><br />
		

		<?php echo T_("HNR2"); ?>: <?php echo $hnr; ?><br />

   <b><?php echo T_("OS"); ?>:</b> <?php echo $CURUSER["os"]."&nbsp;&nbsp;<img src='".$site_config['SITEURL']."/images/oss/".$CURUSER["os"].".png' width='40' height='40'";?><br />
   <b><?php echo T_("BROWSER"); ?>:</b> <?php echo $browser?><br />
<br />
	</td></tr>
	</table>
	<br /><br />
	<?php
	end_frame();
	   $var = explode( ' ', 'avatar-20 gender-20 client-20 signature-20 title-20' );
  
        $data = array();
        foreach ( $var as $k => $v )
        {
                $var = explode( '-', $v );
                $data[$var[ 0 ]] = ( int ) $var[ 1 ];
        }
        ...
        foreach ( $data as $k => $v )
        {
                ...
                {
                         $text .= '<b>' . ucwords( $k ) . '</b> (+' . $v . '%) ';
                }
        }
        
        begin_frame( T_('PRO_COMPLETE') );
        
        if ( $text != null )
        {
                 echo sprintf( T_('PRO_COMPLETE_INFO'), $text );
        }
        ...
        <?php echo sprintf( T_('PRO_PERCENT'), ( int ) $percentage ); ?>
        <div class="profile_bar"><span style="width: <?php echo ( int ) $percentage; ?>%; "></span> </div>
...
?>
    ...
    <tr>
        ...
        <th class="table_head"><?php echo T_("COMMENTS"); ?></th>
        <th class="table_head"><?php echo T_("HITS"); ?></th>
...
if ($action=="edit_settings"){
	...
	while ($sasha = mysqli_fetch_array($sashok))
		$teams .= "<option value='$sasha[id]'" . ($CURUSER["team"] == $sasha['id'] ? " selected='selected'" : "") . ">$sasha[name]</option>\n"; 


	$acceptpms = $CURUSER["acceptpms"] == "yes";
	print ("<tr><td align='right' class='table_col2'><b>" . T_("ACCEPT_PMS") . ":</b> </td><td class='table_col2'><input type='radio' name='acceptpms'" . ($acceptpms ? " checked='checked'" : "") .
	...
	print("<tr><td align='right' class='table_col2'><b>" . T_("EMAIL_NOTIFICATION") . ":</b> </td><td align='left' class='table_col2'><input type='checkbox' name='pmnotif' " . (strpos($CURUSER['notifs'], "[pm]") !== false ? " checked='checked'" : "") .
	...
	if ($CURUSER["class"] > 1)
		print("<tr><td align='right' class='table_col2'><b>".T_("TEAM").":</b> </td><td align='left' class='table_col2'><select name='teams'>\n$teams\n</select></td></tr>");
...
if (!$avatar)
   ...
   print("<tr><td align='right' class='table_col1'><b>" . T_("AVATAR") . ":</b></td><td align='left' class='table_col1'><table><tr><td><img src=$avatar height='40' width='40'></td><td ><input type='text' name='avatar' size='50' value=\"" . htmlspecialchars($CURUSER["avatar"]) . "\"><br /><i>" . T_("AVATAR_LINK") . "</i>&nbsp;or <a href=upavatar.php?action=upload><b>" . T_("UP_AVATAR") . "</b></a></td></tr></table></td></tr>");
   }
   else {
   print("<tr><td align='right' class='table_col1'><b>" . T_("AVATAR_UPLOAD") . ":</b> </td><td align='left' class='table_col1'><input type='text' name='avatar' size='50' value=\"" . htmlspecialchars($CURUSER["avatar"]) .
     "\" /><br />\n<i>".T_("AVATAR_LINK")."</i><br /></td></tr>");
	}
///END UPLOAD AVATAR ///
	print("<tr><td align='right' class='table_col2'><b>" . T_("CUSTOM_TITLE") . ":</b> </td><td align='left' class='table_col2'><input type='text' name='title' size='50' value=\"" . strip_tags($CURUSER["title"]) .
	  "\" /><br />\n <i>" . T_("HTML_NOT_ALLOWED") . "</i></td></tr>");
	print("<tr><td align='right' class='table_col1' valign='top'><b>" . T_("SIGNATURE") . ":</b> </td><td align='left' class='table_col1'><textarea name='signature' cols='50' rows='10'>" . htmlspecialchars($CURUSER["signature"]) .
	  "</textarea><br />\n <i>".sprintf(T_("MAX_CHARS"), 150).", " . T_("HTML_NOT_ALLOWED") . "</i></td></tr>");

	print("<tr><td align='right' class='table_col2'><b>".T_("RESET_PASSKEY").":</b> </td><td align='left' class='table_col2'><input type='checkbox' name='resetpasskey' value='1' />&nbsp;<i>".T_("RESET_PASSKEY_MSG").".</i></td></tr>");

    print("<tr><td align='right' class='table_col1'><b>Hide Shoutbox:</b></td><td align='left' class='table_col1'><input type='checkbox' name='hideshoutbox' value='yes' ".($CURUSER['hideshoutbox'] == 'yes' ? 'checked="checked"' : '')." />&nbsp;<i>I would like to hide the shoutbox.</i></td></tr> ");
...
autolink("account.php", "<b><center><font color='#ff0000'>Updated OK</font></center></b>");
			if ($changedemail) {
				echo "<br /><center><b>".T_("EMAIL_CHANGE_SEND")."</b></center><br /><br />";
			}
		  }else{
			echo "<br /><br /><center><b><font color='#ff0000'>".T_("ERROR").": ".$message."</font></b></center><br /><br />";
		  }// message
