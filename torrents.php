...
//
//  TorrentTrader v2.x
//      $LastChangedDate: 2011-11-14 21:57:09 +0000 (Mon, 14 Nov 2011) $
...
//get sql info
if ($count) {
	list($pagertop, $pagerbottom, $limit) = pager(40, $count, "torrents.php?" . $addparam);
	$query = "SELECT  torrents.sticky, torrents.id, torrents.anon, torrents.uplreq, torrents.announce, torrents.category, torrents.leechers, torrents.nfo, torrents.seeders, torrents.name, torrents.times_completed, torrents.size, torrents.added, torrents.comments, torrents.numfiles, torrents.filename, torrents.owner, torrents.external, torrents.freeleech, torrents.imdb, torrents.tube, torrents.trailers, categories.name AS cat_name, categories.parent_cat AS cat_parent, categories.image AS cat_pic, users.username, users.privacy, IF(torrents.numratings < 2, NULL, ROUND(torrents.ratingsum / torrents.numratings, 1)) AS rating FROM torrents LEFT JOIN categories ON category = categories.id LEFT JOIN users ON torrents.owner = users.id $where $orderby $limit";
