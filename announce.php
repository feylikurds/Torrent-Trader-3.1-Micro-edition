...
if (preg_match("/^Mozilla|^Opera|^Links|^Lynx/i", $agent))
	die("Eat my cunt. Why are you doing this? None of this works this way");
...
foreach (array("passkey") as $x)
        if (!isset($$x) && $MEMBERSONLY)
                err("Missing key: $x");
...
$torrentfields = "id, info_hash, banned, freeleech, seeders + leechers AS numpeers, UNIX_TIMESTAMP(added) AS ts, seeders, leechers, times_completed, size"; //torrent details to get
...
$res = SQL_Query_exec("SELECT $torrentfields FROM torrents WHERE info_hash=".sqlesc($info_hash));
...
//$res = SQL_Query_exec("SELECT $peerfields FROM peers WHERE torrent = $torrentid $limit");
if ($site_config["cache_peers"]) {
$rows = SQL_Query_exec_cached("SELECT $peerfields FROM peers WHERE torrent = $torrentid $limit", $site_config["cache_peers_time"]);
} else {
$res = SQL_Query_exec("SELECT $peerfields FROM peers WHERE torrent = $torrentid $limit") or err("Error Selecting Peers");
$rows = array();
...
////////////////H&R SYSTEM EDITS///////////////////////////////////////////////
//	if ( $_GET[ 'event' ] != 'stopped' && $site_config["ENABLE_HNR_SYSTEM"] == true)
//{
...
//    if ( ( $MEMBERSONLY ) && ( $seeder == 'no' ) && ( $torrent['freeleech'] == 0 ) ) {
    ...
    if ( ( $MEMBERSONLY ) && ( $seeder == 'no' ) ) {
         SQL_Query_exec("INSERT DELAYED INTO `snatched` (`uid`, `tid`, `stime`, `utime`) VALUES ('$userid', '$torrentid', '".gmtime()."', '".gmtime()."') ON DUPLICATE KEY UPDATE `utime` = '" . gmtime() . "'");
} else {
         SQL_Query_exec("INSERT DELAYED INTO `snatched` (`uid`, `tid`, `stime`, `utime`, `dload`, `completed`) VALUES ('$userid', '$torrentid', '".gmtime()."', '".gmtime()."', '".$tsize."', '1') ON DUPLICATE KEY UPDATE `utime` = '" . gmtime() . "'");
}
